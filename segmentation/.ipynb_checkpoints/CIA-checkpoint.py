...
#          #7.31    添加lab_color_norm层（加得不太对因为bgr空间的数据input已经被归一化了）
#             for batch in enumerate(loader_train):
#                 unorm = UnNormalize(mean=data_config['mean'], std=data_config['std'])#8.2反归一化
#                 #print(batch)
#                 (num, matrix) = batch
#                 [input0,target] = matrix
                
#                 for i in range(0,args.batch_size):
#                     input0[i] = unorm(input0[i])
#                     input0[i] = 255*input0[i]
# #                     print(input0[i])
#                     input0_ = input0[i].permute(1,2,0)
#                     input0_n = input0_.data.cpu().numpy()
#                     #print(input0_n.shape)
#                     input0_n = cv.cvtColor(input0_n, cv.COLOR_RGB2LAB)                    
#                     input0_t = torch.Tensor(input0_n).float().cuda()
#                     input0[i] = input0_t.permute(2,0,1)
                   
#                 m = nn.InstanceNorm2d(3, eps=1e-05, momentum=0.1, affine=True, track_running_stats=False)
#                 output0 = m(input0)
#                 for i in range(0,args.batch_size):
# #                     output0[i]=torch.from_numpy(output0[i])
#                     output0_ = output0[i].permute(1,2,0)
#                     output0_n=output0_.data.cpu().numpy()
#                     output0_n = cv.cvtColor(output0_n, cv.COLOR_LAB2RGB)
#                     output0_t = torch.Tensor(output0_n).float().cuda()
#                     output0[i] = output0_t.permute(2,0,1)
                                
#                 batch = (num,[output0,target])
