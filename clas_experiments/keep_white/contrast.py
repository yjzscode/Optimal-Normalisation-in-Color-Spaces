...
class ImageContraster():
    ...
    def enhance_contrast(self, img, method="HE", level=256, window_size=32, affect_size=16, blocks=8, threshold=10.0):
        ...
        if channel_num == 1:
            ...
            img_arr[:, :, 2] = he_func(img_arr[:, :, 2], level=level, window_size=window_size, affect_size=affect_size,
                                       blocks=blocks, threshold=threshold)
            img_res = cv2.cvtColor(img_arr, cv2.COLOR_HSV2BGR)
    ...
    def plot_images(self, img1, img2):
        ...
        # plt.imshow(img2)
        # plt.show()
        cv2.imwrite('CLAHE.png', cv2.cvtColor(img2, cv2.COLOR_BGR2RGB))
