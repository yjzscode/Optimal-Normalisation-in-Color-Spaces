...
from torchvision.models import resnet18, resnet18_13,resnet18_2,resnet18_6,resnet18_8,resnet18_9,resnet18_mean,resnet18_lab_norigin,resnet18_lab_norm_6,resnet18_lab_norm_8,resnet18_lab_norm_mean,resnet18_hsv_norm,resnet18_hed_norm,resnet18_concat,resnet18_avg,resnet18_weighted_avg, resnet50, mobilenet_v3_large, mobilenet_v3_small#, efficientnet_b0 9.22
...
def main():
    ...
    elif args.model == 'resnet18_6':
        model = resnet18_6(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_6 \n')
    elif args.model == 'resnet18_9':
        model = resnet18_9(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_9 \n')
    elif args.model == 'resnet18_2':
        model = resnet18_2(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_2 \n')
    elif args.model == 'resnet18_13':
        model = resnet18_13(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_13 \n')
    elif args.model == 'resnet18_8':
        model = resnet18_8(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_8 \n')
    elif args.model == 'resnet18_mean':
        model = resnet18_mean(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_mean \n')
        
    elif args.model == 'resnet18_lab_norigin':#9.11
        model = resnet18_lab_norm_norigin(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_lab_norigin \n')     
    elif args.model == 'resnet18_lab_norm_6':#9.11
        model = resnet18_lab_norm_6(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_lab_norm_6 \n') 
    elif args.model == 'resnet18_lab_norm_8':#9.11
        model = resnet18_lab_norm_8(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_lab_norm_8 \n') 
    elif args.model == 'resnet18_lab_norm_mean':#9.11
        model = resnet18_lab_norm_mean(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_lab_norm_mean \n') 
        
    elif args.model == 'resnet18_hed_norm':#9.11
        model = resnet18_hed_norm(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_hed_norm \n')
    elif args.model == 'resnet18_hsv_norm':#9.11
        model = resnet18_hsv_norm(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_hsv_norm \n')
        
    elif args.model == 'resnet18_concat':#9.11
        model = resnet18_concat(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_concat \n')
    elif args.model == 'resnet18_avg':#9.11
        model = resnet18_avg(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_avg \n')
    elif args.model == 'resnet18_weighted_avg':#9.11
        model = resnet18_weighted_avg(num_classes=args.num_classes)
        _logger.info('torchvision.resnet18_weighted_avg \n')
...
#         logger = _logger
    ...
    try:
        ...
        for epoch in range(start_epoch, args.epochs): #1.21修改，直接改成num_epochs->args.epochs，0-49
            ...
            #12.20修改，增加test评估
            test_metrics = test_and_norm(epoch,model, loader_test, validate_loss_fn, args) 
            
#             list.append(model.bw.mu.detach().cpu().numpy().tolist()+model.bw.sigma.detach().cpu().numpy().tolist())
...
#         column=['EL','EA','EB','VL','VA','VB'] #列表头名称
#         test=pd.DataFrame(columns=column,data=list)#将数据放进表格
#         test.to_csv(r'/root/autodl-tmp/RandStainNA-master/classification/output/train/100K_TEMmean/test.csv') #数据存入csv,存储位置及文件名称
...
def test_and_norm(epoch,model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            output_norm = model.bw(input) #10.29
            output_norm = norm4D(output_norm,mean_,std_)
            path1='/root/autodl-tmp/RandStainNA-master/classification/output/train/100K_LAB_mean_ep2/norm_results/norm'
            path2='/root/autodl-tmp/RandStainNA-master/classification/output/train/100K_LAB_mean_ep2/norm_results/origin'
            mkdir(path1)
            mkdir(path2)
            torchvision.utils.save_image(
                output_norm,
                os.path.join(path1, 'test-batch-ep%d-%d.jpg' % (epoch,batch_idx)),
                padding=0,
                normalize=False)
            torchvision.utils.save_image(
                input,
                os.path.join(path2,'test-batch-ep%d-%d.jpg' % (epoch,batch_idx)),
                padding=0,
                normalize=False)
