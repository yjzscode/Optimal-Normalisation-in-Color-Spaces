...
from CIAnet_bw_origin_weighted_avg import *
import scipy.io
...
# mean_0 = (50, 30, -10) #8.8添加lab_norm初始值，依据是chosen template中表现最好的一张，之后可用数据集平均值
# std_0 = (20, 10, 10)

mean_hsv = (4.8756,0.4016,0.5936) #9.11添加hsv_norm初始值，依据是chosen template中表现最好的一张，之后可用数据集平均值
...
#         print('output0',output0)
        ...
        for i in range(0, x.shape[0]):
            for j in range(0, x.shape[1]):
                output[i][j] = (output0[i][j].to(device) * self.weight0_hsv[j] + self.bias0_hsv[j])*std_hsv[j]+mean_hsv[j]#list
            output[i] = torch.stack(output[i])
        output = torch.stack(output)
#         print(output)
#         print(self.weight0,self.bias0)
        xxx = hsv_to_rgb(output)
#         print(xxx)
        x4 = xxx
        for i in range(0, x.shape[0]):
            for j in range(0, x.shape[1]):
...
#         print(xxx)
        ...
        for i in range(0, x.shape[0]):
            for j in range(0, x.shape[1]):
                x4_lab[i][j] = ((xxx_lab[i][j].to(device) -mean_[j])/std_[j])*self.w1[j]
                x4_hsv[i][j] = ((xxx_hsv[i][j].to(device) -mean_[j])/std_[j])*self.w2[j]
                x4_hed[i][j] = ((xxx_hed[i][j].to(device) -mean_[j])/std_[j])*(1-self.w1[j]-self.w2[j])
                x4[i][j] = x4_lab[i][j] + x4_hsv[i][j] + x4_hed[i][j]#9.29
        return x4

    
class CIAnet_bw_origin_weighted_avg(nn.Module):
    def __init__(self, growthRate, nDenseBlocks, reduction, bottleneck):
        super(CIAnet_bw_origin_weighted_avg, self).__init__()               
        self.bw = bw()
