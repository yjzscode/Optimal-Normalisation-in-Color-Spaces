...
from CIAnet_bw_origin_average import *
import scipy.io
...
#         print(xxx)
        ...
        for i in range(0, x.shape[0]):
            for j in range(0, x.shape[1]):
                ...
                x4[i][j] = (x4_lab[i][j] + x4_hsv[i][j] + x4_hed[i][j])*(1/3) #9.29
        return x4

    
class CIAnet_bw_origin_average(nn.Module):
    def __init__(self, growthRate, nDenseBlocks, reduction, bottleneck):
        super(CIAnet_bw_origin_average, self).__init__()               
        self.bw = bw()
