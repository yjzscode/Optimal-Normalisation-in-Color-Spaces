...
if __name__ == '__main__':
    ...
    base_dir = '/root/autodl-tmp/RandStainNA-master/segmentation/outputs'
...
#                     )
    ...
    if args.model == 'CIAnet_1':
        model = CIAnet_1(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    
    elif args.model == 'CIAnet_2':#9.12
        model = CIAnet_2(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_3':#9.12
        model = CIAnet_3(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_4':#9.12
        model = CIAnet_4(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_5':#9.12
        model = CIAnet_5(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_mean':#9.12
        model = CIAnet_mean(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_lab_norigin':#9.12
        model = CIAnet_lab_norigin(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_lab_2':#9.12
        model = CIAnet_lab_2(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_lab_3':#9.12
        model = CIAnet_lab_3(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_lab':#9.12
        model = CIAnet_lab(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_hsv':#9.12
        model = CIAnet_hsv(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_hed':#9.12
        model = CIAnet_hed(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_concat':#9.12
        model = CIAnet_concat(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_avg':#9.12
        model = CIAnet_avg(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    elif args.model == 'CIAnet_weighted_avg':#9.12
        model = CIAnet_weighted_avg(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
    else:
        model = CIAnet_weighted_avg(growthRate=6, nDenseBlocks=[6,12,24,16], reduction=0.5, bottleneck=True).to(device)
...
#             print(batch)
            ...
            with torch.no_grad():
                for verify in tqdm(test_loader):
                    ...
                    if save_idx == 1 :
                        ...
                        imgsave = img[0][0].cpu().clone()#10.29 展示原图像
                        imgsave = imgsave.squeeze(0) # 压缩一维
                        imgsave = transforms.ToPILImage()(imgsave) # 自动转换为0-255
                        imgsave.save(os.path.join(o_dir,'test/data_test.png'))
                    img = img.to(device)
                    label = label.to(device)
                    bound = bound.to(device)
    ...
    print(model.state_dict()['bw_lab_norigin.weight'])#10.24
