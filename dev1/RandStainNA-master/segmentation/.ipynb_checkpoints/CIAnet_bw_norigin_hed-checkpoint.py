...
from CIAnet_bw_origin_hed import *
import scipy.io
...
from kornia.color import lab_to_rgb,rgb_to_lab#8.28添加
from hed import hed_to_rgb,rgb_to_hed#9.11

device = 'cuda:0' if torch.cuda.is_available() else 'cpu'
mean_ = (0.485, 0.456, 0.406)
std_ = (0.229, 0.224, 0.225)
# mean_0 = (59.331425, 31.336252, -5.631718) #8.8添加lab_norm初始值，依据是chosen template中表现最好的一张，之后可用数据集平均值
# std_0 = (22.208018, 13.605981, 8.484636)

# mean_0 = (50, 30, -10) #8.8添加lab_norm初始值，依据是chosen template中表现最好的一张，之后可用数据集平均值
# std_0 = (20, 10, 10)

mean_0 = (0.6411, -0.3809, 0.5712) #8.8添加hed_norm初始值，依据是chosen template中表现最好的一张，之后可用数据集平均值
std_0 = (0.3239, 0.0990, 0.1606)
...
#         print('output0',output0)
        ...
        for i in range(0, x.shape[0]):
            for j in range(0, x.shape[1]):
                output[i][j] = output0[i][j].to(device) * self.weight0[j] + self.bias0[j]
...
class CIAnet_bw_norigin_hed(nn.Module):
    def __init__(self, growthRate, nDenseBlocks, reduction, bottleneck):
        super(CIAnet_bw_norigin_hed, self).__init__()               
        self.bw = bw()
